...
S1 必须在不改动 Service V3 公共协议与 codec 的前提下，建立 SCM 生命周期、Pipe 本机安全边界和可测的
安装/恢复/卸载入口。后续产品决策明确不做调用方身份认证；Pipe ACL 只负责限定本机传输权限。
...
2. Accept 后直接建立 Service session，不查询客户端进程 token，不读取 SID/session/admin 标志，也不执行
   allowlist 或身份授权。Service 生成仅用于连接内资源绑定的随机/唯一 session id。
3. Service Host 提供 `run_service_host`：在本机 accept 上运行 Service session；取消会唤醒 pending
   accept/receive。
4. SCM 生命周期通过 `IWindowsServiceControlManager` 暴露：
...
5. `WindowsServiceScmHost` 在独立线程运行 worker：`request_stop` 后在 `stop_deadline` 内等待；
   超时则报告 Stopped 并返回 `kInternal`（非协作 worker 被 detach，由进程退出回收）。
6. 不修改 Service V3 协议字段、codec、SQLite、Repository 或 Worker Supervisor。

## 备选方案

- Interactive/admin 身份认证或 SID allowlist：产品不需要认证，且会把用户 token 语义扩散到业务层，不采用。
...
- Named Pipe：process-default 连接、本机 ACL、accept 取消；源码不查询 peer identity。
- Service host：accept/session 成功、stop 取消 pending accept。
