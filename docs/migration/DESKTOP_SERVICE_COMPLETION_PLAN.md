...
| ID | 优先级 | 工作包 | 主要产物 | 前置 |
| --- | --- | --- | --- | --- |
| D0 | P0 | Desktop 国际化基础 | Linguist、LocaleController、语言设置、门禁测试 | 无 |
| D1 | P0 | Desktop 客户端分层 | transport、request coordinator、models、协议适配边界 | 无 |
| S0 | P0 | Service 协议 V3（已完成） | commands/queries/events DTO、codec、ADR | 无 |
| S1 | P1 | 正式 Windows Service 边界 | SCM、Pipe ACL、调用方身份、安装生命周期 | S0 |
| S2 | P1 | SQLite 控制面 | schema、repository connection、job/schedule/event stores | S0 |
| S3 | P1 | Worker Supervisor | 启动、进度、取消、崩溃回收、结果持久化 | S0, S2 |
| S4 | P1 | Inventory 与 Repository API | source inventory、connection/test、recovery point queries | S0, S2 |
| D2 | P1 | Home 与全局任务体验 | Home、Splash、toast、task model | D0, D1, S0 |
| D3 | P1 | Backup 页面 | source/target/options、启动/取消/进度 | D0, D1, S3, S4 |
| S5 | P2 | Archive 深度操作 | authenticate、verify、显式链解析、删除计划 | S2, S3, S4 |
| S6 | P2 | Restore 编排 | preflight、目标检查、任务监督、结果查询 | S3, S5 |
| D4 | P2 | Restore 页面 | recovery point、链、目标、确认、进度 | D0, D1, S6 |
| D5 | P2 | Repository 管理 | add/import/test/default/delete/verify actions | D0, D1, S4, S5 |
| S7 | P3 | Mount Host 编排 | mounted session、unmount、崩溃清理 | S3, S5 |
| D6 | P3 | Mount 页面 | mount/unmount/session table | D0, D1, S7 |
| S8 | P3 | Schedule 与 Event/Audit | trigger engine、history、paged queries | S2, S3 |
| D7 | P3 | Event Log 页面 | filter、分页、详情、导出入口 | D0, D1, S8 |
| D8 | P3 | Settings 页面 | language、theme、service/repository settings | D0, D1, S1, S4 |
| R0 | Gate | 发布工程 | installer、upgrade/uninstall、recovery、diagnostics、E2E | 全部发布范围 |
...
- 提供 SCM 安装、启动、停止、恢复策略和受控卸载入口。
- 使用显式本地 Pipe ACL，拒绝远程 client，校验调用方 session/SID 和授权角色。
- Service 运行身份可访问所需系统能力，但不把高权限句柄交给 Desktop。
- 停止流程取消 accept、会话、计划触发和受监督任务，并在 deadline 内完成收口。
- 测试交互用户、非授权用户、多 session、Service 重启和 installer rollback。

### S2：个人版 SQLite 控制面

- 增加端口和 SQLite adapter，保存 Repository connection、SecretRef、Job、Schedule、Event/Audit 和 schema version。
...
- Service 启动时把遗留 running 任务收敛为 interrupted，并由策略决定是否允许重新提交。
