...
当前实现不使用 packed C++ struct 直接映射外部字节。`.bhx` Sidecar 采用固定 96 字节头和显式小端 payload codec；DATA 使用 SHA-256，ZERO/SKIP hash 全零。V6 Header codec 区分非分卷、首卷和续卷规则，并校验全量/增量 `parent_uuid`。Adapter 已实现完整 chunk 边界分卷、末卷 Footer、稀疏增量层和链式覆盖读取。DEDUP 写入、差异备份和多 volume Archive 仍是后续工作。
