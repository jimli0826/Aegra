...
| Mount 阶段 | `check_dokan` → 打开 Archive 链（volume_set 记录 `range_cache_budget`、`range_prefetch_threads` 与链几何：`base_chunks`、`base_logical_bytes`、`base_chunks_not_decoded`、`overlay_chunks`、`overlay_logical_bytes`、`overlay_slices_raw`、`overlay_slices_visible`）→ volume_set `open_whole_disk_reader` + `present_and_attach_virtual_disk`，或 file_set `mount_file_namespace` → `mounted_session` → `unmount_and_cleanup`（volume_set 记录整盘读取计数 `read_calls`、`bytes_read`；链读取拆分：`chain_range_reads`、`chain_range_bytes`、`chain_piece_cache_hits`、`chain_piece_load_waits`、`chain_base_decodes`、`chain_base_decode_ms`、`chain_overlay_decodes`、`chain_overlay_decode_ms`、`chain_on_demand_decodes`、`chain_on_demand_decode_ms`、`chain_prefetch_decodes`、`chain_prefetch_decode_ms`、`chain_prefetch_failures`、`chain_cache_evictions`、`chain_prefetch_unused`、`chain_prefetch_consumed`、`chain_prefetch_throttled_plans`；各层阶段：`layer_chunks_decoded`、`layer_stored_bytes_read`、`layer_stored_read_ms`、`layer_authenticate_ms`、`layer_expand_ms`）；VHDX 失败在本机日志保留 Win32 数字错误码，IPC 只返回稳定 `mount.*` code |
