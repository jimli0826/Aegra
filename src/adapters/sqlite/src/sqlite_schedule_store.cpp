...
base::Result<void> ScheduleStore::upsert(const ports::ScheduleRecord& record,
    ...
    if (auto bound = stmt.bind_text(6, record.repository_connection_id); !bound) {
        return bound;
    }
    if (auto bound = stmt.bind_int64(7, static_cast<std::int64_t>(record.backup_type)); !bound) {
        return bound;
    }
    if (auto bound = stmt.bind_int64(8, static_cast<std::int64_t>(record.trigger.kind)); !bound) {
        return bound;
    }
    if (auto bound =
            stmt.bind_text(9, encode_local_minutes_of_day(record.trigger.local_minutes_of_day));
        !bound) {
        return bound;
    }
    if (auto bound = stmt.bind_int64(10, record.trigger.weekday_mask); !bound) {
        return bound;
    }
    if (auto bound = stmt.bind_int64(11, record.trigger.day_of_month_mask); !bound) {
        return bound;
    }
    ...
    if (auto bound = stmt.bind_int64(14, record.exclude_page_and_hibernation_files ? 1 : 0);
        !bound) {
        return bound;
    }
    ...
    if (record.content_kind == contracts::ContentKind::kFileSet) {
        ...
        auto cleared = replace_schedule_file_selections(state_.db, record.schedule_id, {});
        if (!cleared) {
            return cleared;
        }
    }
    ...
    while (true) {
        ...
        if (page.items.size() >= request.page.maximum_results) {
            page.continuation_token = encode_page_token(
                kPageScopeSchedules, filter, last_full->created_utc_ms, last_full->schedule_id);
            break;
        }
...
base::Result<void> ScheduleStore::remove(const std::string_view schedule_id,
    ...
    auto statement =
        SqliteStatement::prepare(state_.db, "DELETE FROM schedules WHERE schedule_id = ?");
    if (!statement) {
        return base::Result<void>::failure(statement.error());
    }
    if (auto bound = statement.value().bind_text(1, schedule_id); !bound) {
