#include "aegra/adapters/windows_disk/windows_disk.h"

#include "windows_api.h"

#include <Windows.h>
#include <winioctl.h>

#include <algorithm>
#include <array>
#include <cwctype>
#include <limits>
#include <string>
#include <string_view>
#include <utility>
#include <vector>

namespace aegra::adapters::windows_disk {
namespace {

constexpr std::wstring_view kVolumePrefix = LR"(\\?\Volume{)";
constexpr std::wstring_view kDevicePrefix = LR"(\\.\)";
constexpr std::wstring_view kGlobalPrefix = LR"(\\?\GLOBALROOT\)";

[[nodiscard]] bool equal_case_insensitive(const std::wstring_view left,
                                          const std::wstring_view right) noexcept {
    return left.size() == right.size() &&
           std::ranges::equal(left, right, [](const wchar_t first, const wchar_t second) {
               return std::towlower(first) == std::towlower(second);
           });
}

[[nodiscard]] bool starts_with_case_insensitive(const std::wstring_view value,
                                                const std::wstring_view prefix) noexcept {
    return value.size() >= prefix.size() &&
           equal_case_insensitive(value.substr(0, prefix.size()), prefix);
}

[[nodiscard]] bool is_hex(const wchar_t value) noexcept {
    return (value >= L'0' && value <= L'9') || (value >= L'a' && value <= L'f') ||
           (value >= L'A' && value <= L'F');
}

[[nodiscard]] bool valid_guid_body(const std::wstring_view body) noexcept {
    constexpr std::array<std::size_t, 4> hyphens{8, 13, 18, 23};
    if (body.size() != 36) {
        return false;
    }
    for (std::size_t index = 0; index < body.size(); ++index) {
        const bool expected_hyphen = std::ranges::find(hyphens, index) != hyphens.end();
        if ((expected_hyphen && body[index] != L'-') || (!expected_hyphen && !is_hex(body[index]))) {
            return false;
        }
    }
    return true;
}

[[nodiscard]] bool is_device_path(const std::wstring_view path) noexcept {
    return starts_with_case_insensitive(path, kDevicePrefix) ||
           starts_with_case_insensitive(path, kGlobalPrefix) ||
           starts_with_case_insensitive(path, kVolumePrefix);
}

[[nodiscard]] base::Result<std::filesystem::path> system_volume_path() {
    std::array<wchar_t, MAX_PATH + 1> windows_directory{};
    const auto length = GetWindowsDirectoryW(windows_directory.data(),
                                             static_cast<UINT>(windows_directory.size()));
    if (length == 0 || length >= windows_directory.size()) {
        return base::Result<std::filesystem::path>::failure(
            detail::win32_error(GetLastError(), "GetWindowsDirectoryW"));
    }
    std::array<wchar_t, MAX_PATH + 1> mount_point{};
    if (!GetVolumePathNameW(windows_directory.data(), mount_point.data(),
                            static_cast<DWORD>(mount_point.size()))) {
        return base::Result<std::filesystem::path>::failure(
            detail::win32_error(GetLastError(), "GetVolumePathNameW"));
    }
    std::array<wchar_t, 64> volume_name{};
    if (!GetVolumeNameForVolumeMountPointW(mount_point.data(), volume_name.data(),
                                           static_cast<DWORD>(volume_name.size()))) {
        return base::Result<std::filesystem::path>::failure(
            detail::win32_error(GetLastError(), "GetVolumeNameForVolumeMountPointW"));
    }
    return base::Result<std::filesystem::path>::success(volume_name.data());
}

[[nodiscard]] base::Result<std::filesystem::path>
path_volume(const std::filesystem::path& source) {
    std::array<wchar_t, MAX_PATH + 1> mount_point{};
    if (!GetVolumePathNameW(source.c_str(), mount_point.data(),
                            static_cast<DWORD>(mount_point.size()))) {
        return base::Result<std::filesystem::path>::failure(
            detail::win32_error(GetLastError(), "GetVolumePathNameW"));
    }
    std::array<wchar_t, 64> volume_name{};
    if (!GetVolumeNameForVolumeMountPointW(mount_point.data(), volume_name.data(),
                                           static_cast<DWORD>(volume_name.size()))) {
        return base::Result<std::filesystem::path>::failure(
            detail::win32_error(GetLastError(), "GetVolumeNameForVolumeMountPointW"));
    }
    return base::Result<std::filesystem::path>::success(volume_name.data());
}

[[nodiscard]] base::Result<DWORD> device_control(const HANDLE handle, const DWORD control,
                                                 void* output, const DWORD output_size,
                                                 const char* operation, void* input = nullptr,
                                                 const DWORD input_size = 0) {
    detail::UniqueHandle event(CreateEventW(nullptr, TRUE, FALSE, nullptr));
    if (!event.valid()) {
        return base::Result<DWORD>::failure(detail::win32_error(GetLastError(), "CreateEventW"));
    }
    OVERLAPPED overlapped{};
    overlapped.hEvent = event.get();
    DWORD returned = 0;
    const auto started =
        DeviceIoControl(handle, control, input, input_size, output, output_size, nullptr,
                        &overlapped);
    const auto error = started ? ERROR_SUCCESS : GetLastError();
    if (!started && error != ERROR_IO_PENDING) {
        return base::Result<DWORD>::failure(detail::win32_error(error, operation));
    }
    if (WaitForSingleObject(event.get(), INFINITE) != WAIT_OBJECT_0 ||
        !GetOverlappedResult(handle, &overlapped, &returned, FALSE)) {
        return base::Result<DWORD>::failure(detail::win32_error(GetLastError(), operation));
    }
    return base::Result<DWORD>::success(returned);
}

[[nodiscard]] base::Result<void> volume_control(const HANDLE handle, const DWORD control,
                                                const char* operation) {
    auto result = device_control(handle, control, nullptr, 0, operation);
    if (!result) {
        return base::Result<void>::failure(result.error());
    }
    return base::Result<void>::success();
}

void unlock_volume(const HANDLE handle) noexcept {
    try {
        static_cast<void>(volume_control(handle, FSCTL_UNLOCK_VOLUME, "FSCTL_UNLOCK_VOLUME"));
    } catch (...) {
    }
}

[[nodiscard]] bool has_win32_error(const base::Error& error, const DWORD value) {
    const auto marker = std::string{"Win32 error "} + std::to_string(value);
    return error.code == base::ErrorCode::kIoFailure &&
           error.message.find(marker) != std::string::npos;
}

[[nodiscard]] base::Result<bool> enable_extended_raw_io(const HANDLE handle) {
    auto result = device_control(handle, FSCTL_ALLOW_EXTENDED_DASD_IO, nullptr, 0,
                                 "FSCTL_ALLOW_EXTENDED_DASD_IO");
    if (!result) {
        if (has_win32_error(result.error(), ERROR_INVALID_FUNCTION) ||
            has_win32_error(result.error(), ERROR_INVALID_PARAMETER)) {
            return base::Result<bool>::success(false);
        }
        return base::Result<bool>::failure(result.error());
    }
    return base::Result<bool>::success(true);
}

[[nodiscard]] base::Result<void> lock_and_dismount_volume(const HANDLE handle) {
    auto lock = volume_control(handle, FSCTL_LOCK_VOLUME, "FSCTL_LOCK_VOLUME");
    if (!lock) {
        if (has_win32_error(lock.error(), ERROR_SHARING_VIOLATION)) {
            return base::Result<void>::failure(
                {base::ErrorCode::kConflict,
                 "target volume is in use (FSCTL_LOCK_VOLUME sharing violation); close open "
                 "handles and retry"});
        }
        if (has_win32_error(lock.error(), ERROR_LOCK_VIOLATION)) {
            return base::Result<void>::failure(
                {base::ErrorCode::kConflict,
                 "target volume is locked by another process (FSCTL_LOCK_VOLUME); retry later"});
        }
        return base::Result<void>::failure(lock.error());
    }
    auto dismount = volume_control(handle, FSCTL_DISMOUNT_VOLUME, "FSCTL_DISMOUNT_VOLUME");
    if (!dismount) {
        unlock_volume(handle);
        return base::Result<void>::failure(dismount.error());
    }
    return base::Result<void>::success();
}

[[nodiscard]] base::Result<std::uint64_t> query_capacity(const HANDLE handle,
                                                         const WindowsBlockSinkKind kind) {
    if (kind == WindowsBlockSinkKind::kStableFile) {
        LARGE_INTEGER size{};
        if (!GetFileSizeEx(handle, &size) || size.QuadPart < 0) {
            return base::Result<std::uint64_t>::failure(
                detail::win32_error(GetLastError(), "GetFileSizeEx"));
        }
        return base::Result<std::uint64_t>::success(static_cast<std::uint64_t>(size.QuadPart));
    }
    GET_LENGTH_INFORMATION length{};
    auto result = device_control(handle, IOCTL_DISK_GET_LENGTH_INFO, &length, sizeof(length),
                                 "IOCTL_DISK_GET_LENGTH_INFO");
    if (!result || result.value() < sizeof(length) || length.Length.QuadPart <= 0) {
        return result ? base::Result<std::uint64_t>::failure(
                            {base::ErrorCode::kIoFailure, "volume length is invalid"})
                      : base::Result<std::uint64_t>::failure(result.error());
    }
    return base::Result<std::uint64_t>::success(static_cast<std::uint64_t>(length.Length.QuadPart));
}

[[nodiscard]] base::Result<ports::BlockDeviceGeometry>
query_geometry(const HANDLE handle, const std::uint64_t capacity_bytes) {
    ports::BlockDeviceGeometry geometry;
    geometry.capacity_bytes = capacity_bytes;
    DISK_GEOMETRY disk{};
    auto result = device_control(handle, IOCTL_DISK_GET_DRIVE_GEOMETRY, &disk, sizeof(disk),
                                 "IOCTL_DISK_GET_DRIVE_GEOMETRY");
    if (!result || result.value() < sizeof(disk) || disk.BytesPerSector == 0) {
        return result ? base::Result<ports::BlockDeviceGeometry>::failure(
                            {base::ErrorCode::kIoFailure, "volume geometry is incomplete"})
                      : base::Result<ports::BlockDeviceGeometry>::failure(result.error());
    }
    geometry.logical_sector_size = disk.BytesPerSector;
    geometry.physical_sector_size = disk.BytesPerSector;
    STORAGE_PROPERTY_QUERY query{};
    query.PropertyId = StorageAccessAlignmentProperty;
    query.QueryType = PropertyStandardQuery;
    STORAGE_ACCESS_ALIGNMENT_DESCRIPTOR alignment{};
    auto alignment_result = device_control(handle, IOCTL_STORAGE_QUERY_PROPERTY, &alignment,
                                           sizeof(alignment), "IOCTL_STORAGE_QUERY_PROPERTY",
                                           &query, sizeof(query));
    if (!alignment_result || alignment_result.value() < sizeof(alignment) ||
        alignment.BytesPerLogicalSector == 0 || alignment.BytesPerPhysicalSector == 0) {
        return alignment_result ? base::Result<ports::BlockDeviceGeometry>::failure(
                                      {base::ErrorCode::kIoFailure,
                                       "volume sector alignment is incomplete"})
                                : base::Result<ports::BlockDeviceGeometry>::failure(
                                      alignment_result.error());
    }
    geometry.logical_sector_size = alignment.BytesPerLogicalSector;
    geometry.physical_sector_size = alignment.BytesPerPhysicalSector;
    const auto logical = geometry.logical_sector_size;
    const auto physical = geometry.physical_sector_size;
    if ((logical & (logical - 1U)) != 0 || (physical & (physical - 1U)) != 0 ||
        physical < logical || capacity_bytes % logical != 0) {
        return base::Result<ports::BlockDeviceGeometry>::failure(
            {base::ErrorCode::kIoFailure, "volume sector geometry is invalid"});
    }
    return base::Result<ports::BlockDeviceGeometry>::success(geometry);
}

[[nodiscard]] base::Result<void>
validate_protected_sources_volume(const WindowsRandomAccessBlockDeviceOpenRequest& request) {
    if (request.protected_sources.empty()) {
        return base::Result<void>::failure(
            {base::ErrorCode::kInvalidArgument, "volume device requires protected sources"});
    }
    for (const auto& source : request.protected_sources) {
        if (source.empty()) {
            return base::Result<void>::failure(
                {base::ErrorCode::kInvalidArgument, "protected source path is empty"});
        }
        auto source_volume = path_volume(source);
        if (!source_volume) {
            return base::Result<void>::failure(source_volume.error());
        }
        if (equal_case_insensitive(request.path.native(), source_volume.value().native())) {
            return base::Result<void>::failure(
                {base::ErrorCode::kConflict, "restore source is located on the target volume"});
        }
    }
    return base::Result<void>::success();
}

[[nodiscard]] base::Result<void>
validate_protected_sources_disk(const WindowsRandomAccessBlockDeviceOpenRequest& request,
                                const std::uint32_t target_disk) {
    if (request.protected_sources.empty()) {
        return base::Result<void>::failure(
            {base::ErrorCode::kInvalidArgument, "disk device requires protected sources"});
    }
    for (const auto& source : request.protected_sources) {
        if (source.empty()) {
            return base::Result<void>::failure(
                {base::ErrorCode::kInvalidArgument, "protected source path is empty"});
        }
        auto source_disk = physical_disk_number_for_path(source);
        if (!source_disk) {
            return base::Result<void>::failure(source_disk.error());
        }
        if (source_disk.value() == target_disk) {
            return base::Result<void>::failure(
                {base::ErrorCode::kConflict, "restore source is located on the target disk"});
        }
    }
    return base::Result<void>::success();
}

[[nodiscard]] base::Result<void>
validate_open_request(const WindowsRandomAccessBlockDeviceOpenRequest& request) {
    if (request.path.empty()) {
        return base::Result<void>::failure(
            {base::ErrorCode::kInvalidArgument, "block device path is empty"});
    }
    if (request.kind == WindowsBlockSinkKind::kVolume) {
        if (!WindowsBlockSink::is_canonical_volume_guid_path(request.path)) {
            return base::Result<void>::failure(
                {base::ErrorCode::kInvalidArgument, "target is not a canonical Volume GUID path"});
        }
        auto system = system_volume_path();
        if (!system) {
            return base::Result<void>::failure(system.error());
        }
        if (equal_case_insensitive(request.path.native(), system.value().native())) {
            return base::Result<void>::failure(
                {base::ErrorCode::kConflict, "online system volume restore is forbidden"});
        }
        return validate_protected_sources_volume(request);
    }
    if (request.kind == WindowsBlockSinkKind::kPhysicalDisk) {
        if (!WindowsBlockSink::is_physical_drive_path(request.path)) {
            return base::Result<void>::failure(
                {base::ErrorCode::kInvalidArgument, "target is not a PhysicalDrive path"});
        }
        auto disk_number = WindowsBlockSink::physical_drive_number(request.path);
        if (!disk_number) {
            return base::Result<void>::failure(
                {base::ErrorCode::kInvalidArgument, "PhysicalDrive number is invalid"});
        }
        auto system = is_system_physical_disk(disk_number.value());
        if (!system) {
            return base::Result<void>::failure(system.error());
        }
        if (system.value()) {
            return base::Result<void>::failure(
                {base::ErrorCode::kConflict, "online system disk restore is forbidden"});
        }
        return validate_protected_sources_disk(request, disk_number.value());
    }
    if (is_device_path(request.path.native())) {
        return base::Result<void>::failure(
            {base::ErrorCode::kInvalidArgument, "stable file device rejects device paths"});
    }
    return base::Result<void>::success();
}

[[nodiscard]] std::filesystem::path open_path_for_request(
    const WindowsRandomAccessBlockDeviceOpenRequest& request) {
    if (request.kind == WindowsBlockSinkKind::kVolume) {
        auto value = request.path.native();
        value.pop_back();
        return value;
    }
    return request.path;
}

[[nodiscard]] base::Result<void>
wait_for_io(const HANDLE handle, OVERLAPPED& overlapped, const DWORD expected,
            const base::CancellationToken& cancellation, const char* incomplete_message) {
    constexpr DWORD kPollMilliseconds = 25;
    for (;;) {
        const auto wait = WaitForSingleObject(overlapped.hEvent, kPollMilliseconds);
        if (wait == WAIT_OBJECT_0) {
            break;
        }
        if (wait != WAIT_TIMEOUT) {
            return base::Result<void>::failure(
                detail::win32_error(GetLastError(), "WaitForSingleObject"));
        }
        if (cancellation.stop_requested()) {
            CancelIoEx(handle, &overlapped);
            WaitForSingleObject(overlapped.hEvent, INFINITE);
            return base::Result<void>::failure(
                {base::ErrorCode::kCancelled, "block device I/O cancelled"});
        }
    }
    DWORD transferred = 0;
    if (!GetOverlappedResult(handle, &overlapped, &transferred, FALSE)) {
        return base::Result<void>::failure(
            detail::win32_error(GetLastError(), "GetOverlappedResult"));
    }
    if (transferred != expected) {
        return base::Result<void>::failure({base::ErrorCode::kIoFailure, incomplete_message});
    }
    return base::Result<void>::success();
}

[[nodiscard]] bool range_aligned(const std::uint64_t offset, const std::size_t size,
                                 const std::uint32_t alignment) noexcept {
    if (alignment <= 1) {
        return true;
    }
    return (offset % alignment) == 0 && (size % alignment) == 0;
}

[[nodiscard]] base::Result<void> check_range(const ports::BlockDeviceGeometry& geometry,
                                             const std::uint64_t offset, const std::size_t size,
                                             const std::uint32_t alignment) {
    if (offset > geometry.capacity_bytes || size > geometry.capacity_bytes - offset ||
        size > (std::numeric_limits<DWORD>::max)()) {
        return base::Result<void>::failure(
            {base::ErrorCode::kInvalidArgument, "block device range is outside capacity"});
    }
    if (!range_aligned(offset, size, alignment)) {
        return base::Result<void>::failure(
            {base::ErrorCode::kInvalidArgument, "block device range is not sector aligned"});
    }
    return base::Result<void>::success();
}

[[nodiscard]] base::Result<void>
prove_last_aligned_block_access(const HANDLE handle, const std::uint64_t capacity_bytes,
                                const std::uint32_t alignment_bytes) {
    if (alignment_bytes == 0 || capacity_bytes < alignment_bytes ||
        capacity_bytes % alignment_bytes != 0) {
        return base::Result<void>::failure(
            {base::ErrorCode::kInvalidArgument, "target block geometry is invalid"});
    }
    std::vector<std::byte> probe(alignment_bytes);
    detail::UniqueHandle event(CreateEventW(nullptr, TRUE, FALSE, nullptr));
    if (!event.valid()) {
        return base::Result<void>::failure(
            detail::win32_error(GetLastError(), "CreateEventW"));
    }
    const auto offset = capacity_bytes - alignment_bytes;
    OVERLAPPED overlapped{};
    overlapped.Offset = static_cast<DWORD>(offset & 0xFFFFFFFFULL);
    overlapped.OffsetHigh = static_cast<DWORD>(offset >> 32U);
    overlapped.hEvent = event.get();
    const auto started = ReadFile(handle, probe.data(), alignment_bytes, nullptr, &overlapped);
    const auto error = started ? ERROR_SUCCESS : GetLastError();
    if (!started && error != ERROR_IO_PENDING) {
        return base::Result<void>::failure(detail::win32_error(error, "ReadFile last block probe"));
    }
    auto waited = wait_for_io(handle, overlapped, alignment_bytes, base::CancellationToken{},
                              "target last block probe was incomplete");
    if (!waited) {
        return base::Result<void>::failure(waited.error());
    }
    return base::Result<void>::success();
}

[[nodiscard]] base::Result<void>
prepare_extended_raw_io(const HANDLE handle, const std::uint64_t capacity_bytes,
                        const std::uint32_t alignment_bytes) {
    auto extended = enable_extended_raw_io(handle);
    if (!extended) {
        return base::Result<void>::failure(extended.error());
    }
    if (extended.value()) {
        return base::Result<void>::success();
    }
    return prove_last_aligned_block_access(handle, capacity_bytes, alignment_bytes);
}

} // namespace

struct WindowsRandomAccessBlockDevice::Impl final {
    detail::UniqueHandle handle;
    ports::BlockDeviceGeometry geometry{};
    std::uint32_t alignment_bytes{512};
    bool volume_locked{false};

    ~Impl() {
        if (volume_locked && handle.valid()) {
            unlock_volume(handle.get());
        }
    }
};

WindowsRandomAccessBlockDevice::WindowsRandomAccessBlockDevice(std::unique_ptr<Impl> impl)
    : impl_(std::move(impl)) {}

WindowsRandomAccessBlockDevice::~WindowsRandomAccessBlockDevice() = default;

base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>
WindowsRandomAccessBlockDevice::open(const WindowsRandomAccessBlockDeviceOpenRequest& request) {
    auto valid = validate_open_request(request);
    if (!valid) {
        return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::failure(
            valid.error());
    }
    const auto share_mode = request.kind == WindowsBlockSinkKind::kStableFile
                                ? FILE_SHARE_READ
                                : (FILE_SHARE_READ | FILE_SHARE_WRITE);
    detail::UniqueHandle handle(CreateFileW(
        open_path_for_request(request).c_str(), GENERIC_READ | GENERIC_WRITE, share_mode, nullptr,
        OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, nullptr));
    if (!handle.valid()) {
        return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::failure(
            detail::win32_error(GetLastError(), "CreateFileW"));
    }
    bool locked = false;
    if (request.kind == WindowsBlockSinkKind::kVolume) {
        auto prepared = lock_and_dismount_volume(handle.get());
        if (!prepared) {
            return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::failure(
                prepared.error());
        }
        locked = true;
    }
    auto capacity = query_capacity(handle.get(), request.kind);
    if (!capacity) {
        return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::failure(
            capacity.error());
    }
    if (request.expected_capacity_bytes &&
        *request.expected_capacity_bytes != capacity.value()) {
        return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::failure(
            {base::ErrorCode::kConflict, "block device capacity changed"});
    }
    auto geometry = query_geometry(handle.get(), capacity.value());
    if (!geometry) {
        return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::failure(
            geometry.error());
    }
    auto alignment = request.required_alignment_bytes != 0 ? request.required_alignment_bytes
                                                           : geometry.value().logical_sector_size;
    if (alignment == 0) {
        alignment = 512;
    }
    if (request.kind == WindowsBlockSinkKind::kVolume) {
        auto raw_io = prepare_extended_raw_io(handle.get(), capacity.value(), alignment);
        if (!raw_io) {
            return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::failure(
                raw_io.error());
        }
    }
    auto impl = std::make_unique<Impl>();
    impl->handle = std::move(handle);
    impl->geometry = geometry.value();
    impl->alignment_bytes = alignment;
    impl->volume_locked = locked;
    return base::Result<std::unique_ptr<WindowsRandomAccessBlockDevice>>::success(
        std::unique_ptr<WindowsRandomAccessBlockDevice>(
            new WindowsRandomAccessBlockDevice(std::move(impl))));
}

ports::BlockDeviceGeometry WindowsRandomAccessBlockDevice::geometry() const noexcept {
    return impl_->geometry;
}

base::Result<std::size_t>
WindowsRandomAccessBlockDevice::read_at(const std::uint64_t offset,
                                        const std::span<std::byte> destination,
                                        const base::CancellationToken cancellation) {
    if (cancellation.stop_requested()) {
        return base::Result<std::size_t>::failure(
            {base::ErrorCode::kCancelled, "block device I/O cancelled"});
    }
    if (destination.empty()) {
        return base::Result<std::size_t>::success(0);
    }
    if (auto range =
            check_range(impl_->geometry, offset, destination.size(), impl_->alignment_bytes);
        !range) {
        return base::Result<std::size_t>::failure(range.error());
    }
    detail::UniqueHandle event(CreateEventW(nullptr, TRUE, FALSE, nullptr));
    if (!event.valid()) {
        return base::Result<std::size_t>::failure(
            detail::win32_error(GetLastError(), "CreateEventW"));
    }
    OVERLAPPED overlapped{};
    overlapped.Offset = static_cast<DWORD>(offset & 0xFFFFFFFFULL);
    overlapped.OffsetHigh = static_cast<DWORD>(offset >> 32U);
    overlapped.hEvent = event.get();
    const auto size = static_cast<DWORD>(destination.size());
    const auto started =
        ReadFile(impl_->handle.get(), destination.data(), size, nullptr, &overlapped);
    const auto error = started ? ERROR_SUCCESS : GetLastError();
    if (!started && error != ERROR_IO_PENDING) {
        return base::Result<std::size_t>::failure(detail::win32_error(error, "ReadFile"));
    }
    auto waited = wait_for_io(impl_->handle.get(), overlapped, size, cancellation,
                              "block device short read rejected");
    if (!waited) {
        return base::Result<std::size_t>::failure(waited.error());
    }
    return base::Result<std::size_t>::success(destination.size());
}

base::Result<void>
WindowsRandomAccessBlockDevice::write_at(const std::uint64_t offset,
                                         const std::span<const std::byte> source,
                                         const base::CancellationToken cancellation) {
    if (cancellation.stop_requested()) {
        return base::Result<void>::failure(
            {base::ErrorCode::kCancelled, "block device I/O cancelled"});
    }
    if (source.empty()) {
        return base::Result<void>::success();
    }
    if (auto range = check_range(impl_->geometry, offset, source.size(), impl_->alignment_bytes);
        !range) {
        return range;
    }
    detail::UniqueHandle event(CreateEventW(nullptr, TRUE, FALSE, nullptr));
    if (!event.valid()) {
        return base::Result<void>::failure(detail::win32_error(GetLastError(), "CreateEventW"));
    }
    OVERLAPPED overlapped{};
    overlapped.Offset = static_cast<DWORD>(offset & 0xFFFFFFFFULL);
    overlapped.OffsetHigh = static_cast<DWORD>(offset >> 32U);
    overlapped.hEvent = event.get();
    const auto size = static_cast<DWORD>(source.size());
    const auto started =
        WriteFile(impl_->handle.get(), source.data(), size, nullptr, &overlapped);
    const auto error = started ? ERROR_SUCCESS : GetLastError();
    if (!started && error != ERROR_IO_PENDING) {
        return base::Result<void>::failure(detail::win32_error(error, "WriteFile"));
    }
    return wait_for_io(impl_->handle.get(), overlapped, size, cancellation,
                       "block device write was incomplete");
}

base::Result<void>
WindowsRandomAccessBlockDevice::flush(const base::CancellationToken cancellation) {
    if (cancellation.stop_requested()) {
        return base::Result<void>::failure(
            {base::ErrorCode::kCancelled, "block device flush cancelled"});
    }
    if (!FlushFileBuffers(impl_->handle.get())) {
        return base::Result<void>::failure(detail::win32_error(GetLastError(), "FlushFileBuffers"));
    }
    return base::Result<void>::success();
}

base::Result<void> WindowsRandomAccessBlockDevice::write_at_verified(
    const std::uint64_t offset, const std::span<const std::byte> source,
    const base::CancellationToken cancellation) {
    if (auto written = write_at(offset, source, cancellation); !written) {
        return written;
    }
    if (auto flushed = flush(cancellation); !flushed) {
        return flushed;
    }
    std::vector<std::byte> readback(source.size());
    auto read = read_at(offset, readback, cancellation);
    if (!read) {
        return base::Result<void>::failure(read.error());
    }
    if (!std::equal(source.begin(), source.end(), readback.begin())) {
        return base::Result<void>::failure(
            {base::ErrorCode::kIoFailure, "block device readback mismatch"});
    }
    return base::Result<void>::success();
}

} // namespace aegra::adapters::windows_disk
