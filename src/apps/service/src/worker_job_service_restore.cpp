...
using worker_job_detail::resolve_restore_chain_entries;
using worker_job_detail::RestoreChainLayer;
...
using worker_job_detail::parse_volume_restore_fingerprint;
using worker_job_detail::source_disk_size_from_archive;

struct SourceVolumeManifestInfo final {
    std::uint64_t total_size_bytes{0};
    std::string filesystem;
};

[[nodiscard]] bool is_ntfs_filesystem_name(std::string_view name) noexcept {
    if (name.size() != 4) {
        return false;
    }
    ...
    return lower(name[0]) == 'n' && lower(name[1]) == 't' && lower(name[2]) == 'f' &&
           lower(name[3]) == 's';
}

[[nodiscard]] base::Result<SourceVolumeManifestInfo>
source_volume_info_from_archive(const std::string& archive_path_utf8,
                                const std::uint32_t source_volume_index,
                                const std::string& password) {
    auto path = path_from_utf8(archive_path_utf8);
    if (!path) {
        return base::Result<SourceVolumeManifestInfo>::failure(path.error());
    }
    ...
    if (!reader) {
        return base::Result<SourceVolumeManifestInfo>::failure(reader.error());
    }
    for (const auto& volume : reader.value()->manifest().volumes) {
        if (volume.volume_index == source_volume_index) {
            if (volume.total_size == 0) {
                return base::Result<SourceVolumeManifestInfo>::failure(
                    {base::ErrorCode::kConflict, "source volume size is unavailable"});
            }
            SourceVolumeManifestInfo info;
            info.total_size_bytes = volume.total_size;
            info.filesystem = volume.filesystem;
            return base::Result<SourceVolumeManifestInfo>::success(std::move(info));
        }
    }
    return base::Result<SourceVolumeManifestInfo>::failure(
...
source_volume_size_from_archive(const std::string& archive_path_utf8,
                                ...
                                const std::string& password) {
    auto info =
        source_volume_info_from_archive(archive_path_utf8, source_volume_index, password);
    if (!info) {
        return base::Result<std::uint64_t>::failure(info.error());
    }
    return base::Result<std::uint64_t>::success(info.value().total_size_bytes);
}
...
WorkerJobService::prepare_restore(const contracts::RestorePreflightRequest& request,
    ...
    if (disk_target) {
        if (request.volume_size_policy != contracts::VolumeSizePolicy::kRequireSourceSize) {
            return base::Result<contracts::RestorePreflight>::failure(
                {base::ErrorCode::kInvalidArgument,
                 "disk restore requires volume_size_policy require_source_size"});
        }
        ...
        prepared.message_code = "restore.preflight_ready";
    } else {
        auto volume_info = source_volume_info_from_archive(
            tip_path.value(), request.source_volume_index, request.archive_password);
        if (!volume_info) {
            return base::Result<contracts::RestorePreflight>::failure(volume_info.error());
        }
        const auto volume_size = volume_info.value().total_size_bytes;
        VolumeRestoreChain chain;
        chain.source_volume_index = request.source_volume_index;
        chain.volume_size_bytes = volume_size;
        chain.layers.reserve(chain_entries.value().size());
        for (const auto& entry : chain_entries.value()) {
            chain.layers.push_back({entry.archive_main_key, entry.file_uuid});
        }
        prepared.logical_size_bytes = volume_size;
        prepared.chain_fingerprint = make_volume_restore_fingerprint(chain);
        prepared.minimum_target_bytes = volume_size;
        if (target_capacity >= volume_size) {
            prepared.volume_size_policy = contracts::VolumeSizePolicy::kRequireSourceSize;
            prepared.feasibility = contracts::RestoreFeasibility::kEligible;
            prepared.message_code = "restore.preflight_ready";
        } else if (request.volume_size_policy !=
                   contracts::VolumeSizePolicy::kAllowNtfsRelocation) {
            return base::Result<contracts::RestorePreflight>::failure(
                {base::ErrorCode::kConflict, "restore target is smaller than the source volume"});
        } else if (!is_ntfs_filesystem_name(volume_info.value().filesystem)) {
            return base::Result<contracts::RestorePreflight>::failure(
                {base::ErrorCode::kConflict, "restore.shrink_not_ntfs"});
        } else {
            // Service quick filter only: exact eligibility requires Worker ShrinkPlan (SR4+).
            // Capability restore.ntfs_shrink.v1 stays off until SR8/SR10 gates pass.
            ...
            prepared.minimum_target_bytes = 0;
            prepared.message_code = "restore.shrink_provisional";
        }
    }
...
WorkerJobService::start_restore(const contracts::StartRestoreCommand& command,
    ...
    const auto target_capacity =
        disk_mode ? (target.value().disk_capacity_bytes > 0 ? target.value().disk_capacity_bytes
                                                            : target.value().capacity_bytes)
                  : target.value().capacity_bytes;
    if (record.feasibility != contracts::RestoreFeasibility::kEligible) {
        return base::Result<contracts::CommandAcknowledgement>::failure(
            {base::ErrorCode::kConflict, "restore.shrink_plan_changed"});
    }
    if (target_capacity != record.target_capacity_bytes) {
        return base::Result<contracts::CommandAcknowledgement>::failure(
            {base::ErrorCode::kConflict, "restore.shrink_plan_changed"});
    }
    const bool shrink_allow =
        record.volume_size_policy == contracts::VolumeSizePolicy::kAllowNtfsRelocation;
    if (shrink_allow) {
        if (record.shrink_plan_digest.empty() || record.minimum_target_bytes == 0 ||
            target_capacity < record.minimum_target_bytes) {
            return base::Result<contracts::CommandAcknowledgement>::failure(
                {base::ErrorCode::kConflict, "restore.shrink_plan_changed"});
        }
    } else if (target_capacity < expected_size) {
        return base::Result<contracts::CommandAcknowledgement>::failure(
            {base::ErrorCode::kConflict,
