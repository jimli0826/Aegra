...
base::Error make_plan_error(const base::ErrorCode code, std::string message_code) {
...
std::uint32_t crc32(const std::span<const std::byte> data) noexcept {
    ...
    for (const std::byte b : data) {
        const auto idx = static_cast<std::uint8_t>(crc ^ std::to_integer<std::uint8_t>(b));
        crc = kCrcTable[idx] ^ (crc >> 8);
    }
    return crc ^ 0xFFFFFFFFU;
}

std::string digest_to_hex(const std::span<const std::byte, 32> digest) {
    static constexpr char kHex[] = "0123456789abcdef";
    std::string out;
    out.resize(kDigestHexLen);
    for (std::size_t i = 0; i < 32; ++i) {
        const auto value = std::to_integer<std::uint8_t>(digest[i]);
        out[i * 2U] = kHex[(value >> 4) & 0x0FU];
        out[i * 2U + 1U] = kHex[value & 0x0FU];
    }
    return out;
}

bool is_lowercase_hex64(const std::string_view text) noexcept {
    if (text.size() != kDigestHexLen) {
        return false;
    }
...
void append_u32(std::vector<std::byte>& out, const std::uint32_t value) {
    ...
    append_u16(out, static_cast<std::uint16_t>((value >> 16) & 0xFFFFU));
}
...
void append_u64(std::vector<std::byte>& out, const std::uint64_t value) {
    ...
    append_u32(out, static_cast<std::uint32_t>((value >> 32) & 0xFFFFFFFFU));
}
...
void append_utf8(std::vector<std::byte>& out, const std::string_view text) {
...
void append_utf16(std::vector<std::byte>& out, const std::u16string_view text) {
    append_u32(out, static_cast<std::uint32_t>(text.size()));
    for (const char16_t unit : text) {
        append_u16(out, static_cast<std::uint16_t>(unit));
    }
}

base::Result<void> read_exact(const std::span<const std::byte> data, std::size_t& offset,
                              const std::size_t size, const std::span<std::byte> destination) {
    if (size > destination.size() || offset > data.size() || size > data.size() - offset) {
        return base::Result<void>::failure(
            make_plan_error(base::ErrorCode::kCorruptData, "restore.shrink_plan_corrupt"));
    }
    std::memcpy(destination.data(), data.data() + offset, size);
    offset += size;
    return base::Result<void>::success();
}

base::Result<std::uint16_t> read_u16(const std::span<const std::byte> data, std::size_t& offset) {
    std::byte raw[2];
    auto status = read_exact(data, offset, 2, raw);
    if (!status) {
        return base::Result<std::uint16_t>::failure(status.error());
    }
    std::uint16_t value = 0;
    std::memcpy(&value, raw, sizeof(value));
    return base::Result<std::uint16_t>::success(value);
}

base::Result<std::uint32_t> read_u32(const std::span<const std::byte> data, std::size_t& offset) {
    const auto lo = read_u16(data, offset);
    if (!lo) {
        return base::Result<std::uint32_t>::failure(lo.error());
    }
    const auto hi = read_u16(data, offset);
    if (!hi) {
        return base::Result<std::uint32_t>::failure(hi.error());
    }
    return base::Result<std::uint32_t>::success(
        static_cast<std::uint32_t>(lo.value()) |
        (static_cast<std::uint32_t>(hi.value()) << 16));
}

base::Result<std::uint64_t> read_u64(const std::span<const std::byte> data, std::size_t& offset) {
    const auto lo = read_u32(data, offset);
    if (!lo) {
        return base::Result<std::uint64_t>::failure(lo.error());
    }
    const auto hi = read_u32(data, offset);
    if (!hi) {
        return base::Result<std::uint64_t>::failure(hi.error());
    }
    return base::Result<std::uint64_t>::success(
        static_cast<std::uint64_t>(lo.value()) |
        (static_cast<std::uint64_t>(hi.value()) << 32));
}

base::Result<std::string> read_utf8(const std::span<const std::byte> data, std::size_t& offset) {
    const auto len = read_u32(data, offset);
    if (!len) {
        return base::Result<std::string>::failure(len.error());
    }
    if (len.value() > kMaxStringBytes) {
        return base::Result<std::string>::failure(
            make_plan_error(base::ErrorCode::kCorruptData, "ntfs_resize.plan_string_too_large"));
    }
    if (offset > data.size() || len.value() > data.size() - offset) {
        return base::Result<std::string>::failure(
            make_plan_error(base::ErrorCode::kCorruptData, "restore.shrink_plan_corrupt"));
    }
    std::string out;
    ...
    for (std::uint32_t i = 0; i < len.value(); ++i) {
        out[i] = static_cast<char>(std::to_integer<unsigned char>(data[offset + i]));
    }
    offset += len.value();
    return base::Result<std::string>::success(std::move(out));
}

base::Result<std::u16string> read_utf16(const std::span<const std::byte> data,
                                        std::size_t& offset) {
    const auto units = read_u32(data, offset);
    if (!units) {
        return base::Result<std::u16string>::failure(units.error());
    }
    if (units.value() > kMaxUtf16Units) {
        return base::Result<std::u16string>::failure(
            make_plan_error(base::ErrorCode::kCorruptData, "ntfs_resize.plan_string_too_large"));
    }
    std::u16string out;
    out.resize(units.value());
    for (std::uint32_t i = 0; i < units.value(); ++i) {
        const auto unit = read_u16(data, offset);
        if (!unit) {
            return base::Result<std::u16string>::failure(unit.error());
        }
        out[i] = static_cast<char16_t>(unit.value());
    }
    return base::Result<std::u16string>::success(std::move(out));
}
