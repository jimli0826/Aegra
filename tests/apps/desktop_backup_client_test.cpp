...
bool test_codec_inventory_and_commands() {
    ...
    const auto empty_body = QJsonDocument(response(
                                              QStringLiteral("req-empty"), 1, 4, 0,
                                              QStringLiteral("control_plane.ready"),
    ...
    const auto multi = QJsonDocument(
                           response(QStringLiteral("req-multi"), 1, 4, 0,
                                    QStringLiteral("control_plane.ready"),
                                    QJsonObject{
                                        {QStringLiteral("items"),
                                         QJsonArray{source_item(QStringLiteral("vol.aaa"),
                                                                QStringLiteral("System"), true),
                                                    source_item(QStringLiteral("vol.bbb"),
                                                                QStringLiteral("Data"), true),
                                                    source_item(QStringLiteral("vol.ccc"),
    ...
    const auto offline_conn = QJsonDocument(
                                  response(QStringLiteral("req-conn"), 1, 3, 0,
                                           QStringLiteral("control_plane.ready"),
                                           QJsonObject{
                                               {QStringLiteral("items"),
                                                QJsonArray{connection_item(
    ...
    passed &=
        expect(parse_response_root(offline_conn, QStringLiteral("req-conn"), conn_root) &&
    ...
    const auto cancel_body = encode_cancel_job_request(QStringLiteral("req-cancel"),
                                                       QStringLiteral("cancel-key"),
                                                       QStringLiteral("job-abc"));
    ...
    const auto malformed = QJsonDocument(
                               response(QStringLiteral("req-bad"), 1, 4, 0,
                                        QStringLiteral("control_plane.ready"),
                                        QJsonObject{
                                            {QStringLiteral("items"),
                                             QJsonArray{QJsonObject{
                                                 {QStringLiteral("source_id"),
                                                  QStringLiteral("vol.bad")},
...
bool drain_until_kinds(QLocalSocket& socket, QSet<qint64>& seen, const int timeout_ms) {
    ...
    while (timer.elapsed() < timeout_ms && seen.size() < 4) {
        ...
        if (kind == 2) {
            ...
            send_page(socket, request_id, 3,
                      {connection_item(QStringLiteral("conn-1"), QStringLiteral("Personal"), 1,
                                       true)},
                      std::nullopt);
        } else if (kind == 4) {
            ...
            send_object(socket,
                        response(request_id, 1, 5, 0, QStringLiteral("job.list_ready"),
                                 QJsonObject{{QStringLiteral("items"), QJsonArray{}},
                                             {QStringLiteral("continuation_token"),
                                              QJsonValue(QJsonValue::Null)}}));
        }
...
bool test_service_client_start_cancel_and_observe() {
    ...
    passed &= expect(wait_until([&] { return client.connected() && client.backupStartAvailable() &&
                                             client.inventoryAvailable() &&
                                             client.connectionsAvailable(); },
    ...
    passed &= expect(start_request.value(QStringLiteral("schema_version")).toInteger() == 3 &&
    ...
    passed &= expect(wait_until([&] { return client.activeBackupJobId() ==
                                             QStringLiteral("job-backup-1"); },
                                kShortTimeoutMilliseconds),
    ...
    while (poll_wait.elapsed() < 6'000 && (!answered_job || cancel_request.isEmpty())) {
        ...
        if (kind == 37) {
            ...
            const QJsonObject job{{QStringLiteral("job_id"), QStringLiteral("job-backup-1")},
                                  ...
                                  {QStringLiteral("progress"),
                                   QJsonObject{{QStringLiteral("schema_version"), 1},
                                               ...
                                               {QStringLiteral("message_code"),
                                                QStringLiteral("job.progress")}}},
    ...
    passed &= expect(wait_until([&] { return client.activeBackupCancellable() &&
    ...
    passed &= expect(
        send_object(*socket,
                    response(cancel_request.value(QStringLiteral("request_id")).toString(), 2, 38, 0,
                             QStringLiteral("command.accepted"),
                             command_ack(QStringLiteral("cmd-cancel"), 1,
                                         QStringLiteral("job-backup-1")))),
        "cancel accepted");
    passed &= expect(wait_until([&] { return !client.cancelCommandBusy(); },
                                kShortTimeoutMilliseconds),
