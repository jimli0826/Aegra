...
bool test_request_codec() {
    ...
    bool passed = expect(encoded && encoded.value() ==
                                        R"({"kind":1,"request_id":"request-1","schema_version":1})",
    ...
    passed &= expect(!app::decode_service_request(
                         R"({"schema_version":1,"request_id":"request-1","kind":1,"extra":0})"),
                     "unknown request fields are rejected");
    passed &= expect(
        !app::decode_service_request(R"({"schema_version":1,"request_id":"request-1","kind":"1"})"),
...
bool test_response_codec_and_dispatch() {
    ...
    passed &= expect(
        decoded && decoded.value().request_id == "request-1" && decoded.value().service &&
            decoded.value().service->capabilities == std::vector<std::string>{"service.info"},
...
bool test_structured_rejections() {
    ...
    bool passed = expect(
        invalid_response &&
            invalid_response.value().kind == contracts::ServiceResponseKind::kRequestRejected &&
    ...
    auto unsupported = app::handle_service_message(
        R"({"schema_version":2,"request_id":"request-2","kind":1})", runtime_info());
...
int run_tests() {
    return test_request_codec() && test_response_codec_and_dispatch() &&
                   test_structured_rejections() && test_session()
               ? EXIT_SUCCESS
               : EXIT_FAILURE;
}
